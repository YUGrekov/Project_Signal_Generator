...
from address_map import BaseMap
...
class AttribCN():
    ...
    prefix = ['mod_State', 'mod_State_ext', 'ports_State', 'pwl_ID', 'badFrameCounter']
    variable = ['diagCN']
    sign = 'Diag.CNs.'
    design_modul = 'MK-545-010'
    shift_two = ['badFrameCounter']
    ...
    def work_file(self, type_module: str):
        ...
        try:
            ...
            if len(list_addrr) != len(self.variable):
                msg[f'{today} - DevStudio. Map. {param_m.sign} Отсутствует адрес из списка {self.variable}'] = 2
                return msg
            ...
            msg[f'{today} - DevStudio. Map. {param_m.sign} Заполнено'] = 1
            return msg
        except Exception:
            msg[f'{today} - DevStudio. Map. {param_m.sign} Ошибка {traceback.format_exc()}'] = 2
            return msg
...
class RackStateMap(BaseMap):
    ...
    def work_file(self):
        ...
        try:
            ...
            data = self.gen_funct.select_orm(HardWare, None, HardWare.id)
            list_addrr = self._read_address_mb()

            if len(list_addrr) != len(self.variable):
                msg[f'{today} - DevStudio. Map. {RACKSTATE} Отсутствует адрес из списка {self.variable}'] = 2
                return msg
            ...
            for row in data:
                for i in range(len(self.prefix)):
                    ...
                    self.new_element(root, name, address)
            tree.write(path, pretty_print=True)
            msg[f'{today} - DevStudio. Map. {RACKSTATE} Заполнено'] = 1
            return msg
        except Exception:
            msg[f'{today} - DevStudio. Map. {RACKSTATE} Ошибка {traceback.format_exc()}'] = 2
            return msg