...
class Diskrets():
    ...
    def check_table(self):
        ...
        if not result_tb_signals:
            self.msg[f'{today} - SQL. DI. Таблица signals отсутсвует'] = 2
            return False
        else:
            result_row_signals = self.dop_function.check_row_table(T_SIGNALS)
            if not result_row_signals:
                self.msg[f'{today} - SQL. DI. Таблица signals не заполнена'] = 2
                return False

            result_tb_di = self.dop_function.check_table(T_DI)
            if not result_tb_di:
                self.msg[f'{today} - SQL. DI. Таблица di отсутсвует'] = 2
                self.dop_function.new_table_orm(DI)
                self.msg[f'{today} - SQL. DI. Таблица di добавлена'] = 0
            return True
    ...
    def module_calc(self, uso, basket, module):
        ...
        try:
            hw = self.dop_function.where_select(T_HW, f'variable_{module}, tag',
                                                f'''"uso"='{uso}' and "basket"='{basket}' ''', 'id')
            if len(hw) > 1:
                self.msg[f'{today} - SQL. DI. {uso}.A{basket}_{module}, при вычислении номера для pValue и pHealth обнаружено несколько модулей!'] = 2
                return 'NULL'
            return re.findall('\d+', hw[0][0])[0], hw[0][1]
        except Exception:
            self.msg[f'{today} - SQL. DI. {uso}.A{basket}_{module}, номер модуля pValue и pHealth не найден!'] = 1
            return 'NULL', None
    ...
    def process_request(self):
        ...
        for order in range(3):
            data = self.dop_function.select_orm(Signals, where[order],
                                                Signals.description)
    ...
    def check_signal(self, data):
        ...
        for signal in data:
            ...
            coinc = self.dop_function.select_orm(DI,
                                                 (DI.uso == uso) &
                                                 (DI.basket == basket) &
                                                 (DI.module == module) &
                                                 (DI.channel == channel),
    ...
    def add_new_signal(self, signal, num_through, tag):
        ...
        self.dop_function.write_base_orm(list_DI, DI)
        self.msg[f'{today} - SQL. DI. Добавлен новый сигнал: {self.count_row}'] = 1
    ...
    def add_empty_row(self, count_rez):
        ...
        for i in range(self.count_row + 1, self.count_row + count_rez + 1):
            ...
            self.dop_function.write_base_orm(empty_row, DI)
    ...
    def update_table(self, signal):
        ...
        coinc = self.dop_function.select_orm(DI,
                                             ...
                                             DI.basket)
        if not bool(coinc):
            self.dop_function.update_base_orm(DI,
                                              {'tag': signal.tag,
                                               'name': signal.description},
                                              (DI.uso == signal.uso) &
            ...
            self.msg[f'{today} - SQL. DI. Строка обновлена: {self.msg_id}'] = 0
    ...
    def add_diskrets(self):
        try:
            ...
            self.msg[f'{today} - SQL. DI. Работа с таблицей завершена'] = 1
            return self.msg
        except Exception:
            self.msg[f'{today} - SQL. DI. Ошибка {traceback.format_exc()}'] = 2
            return self.msg